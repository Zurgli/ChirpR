...
mod windows_gallery {
    ...
    use windows_sys::Win32::Graphics::Gdi::{
        BeginPaint, BitBlt, CreateCompatibleBitmap, CreateCompatibleDC, DT_CENTER,
        DT_SINGLELINE, DT_VCENTER, DeleteDC, DeleteObject, DrawTextW, EndPaint, HDC,
        InvalidateRect, PAINTSTRUCT, SRCCOPY, SelectObject, SetBkMode, SetTextColor, TRANSPARENT,
        UpdateWindow,
    };
    use windows_sys::Win32::Graphics::GdiPlus::{
        ...
        GdipDeletePen, GdipDrawArcI, GdipDrawEllipseI, GdipDrawLineI, GdipDrawPath,
        GdipFillEllipseI, GdipFillPath, GdipFillRectangleI, GdipGraphicsClear,
        GdipSetCompositingQuality, GdipSetPenLineJoin, GdipSetPixelOffsetMode,
        GdipSetSmoothingMode, GdipSetTextRenderingHint, GdiplusStartup, GdiplusStartupInput,
        GpBrush, GpGraphics, GpPath, LineJoinRound, PixelOffsetModeHalf,
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        ...
        LoadCursorW, MSG, PostQuitMessage, RegisterClassW, SetProcessDPIAware, SetTimer,
        ShowWindow, TranslateMessage, CS_HREDRAW, CS_VREDRAW, CW_USEDEFAULT, IDC_ARROW, SW_SHOW,
        WM_DESTROY, WM_ERASEBKGND, WM_PAINT, WM_SIZE, WM_TIMER, WNDCLASSW,
    ...
    fn render_gallery(hdc: HDC, width: i32, height: i32) {
        unsafe {
            ...
            let elapsed = START_TIME.get().map(|start| start.elapsed().as_secs_f32()).unwrap_or(0.0);
            let cards_top = HEADER_HEIGHT;
            let available_height = (height - cards_top - OUTER_PAD).max(CARD_ROWS * CARD_MIN_HEIGHT);
            let card_width = ((width - OUTER_PAD * 2 - CARD_GAP * (CARD_COLUMNS - 1)) / CARD_COLUMNS).max(180);
            let card_height = ((available_height - CARD_GAP * (CARD_ROWS - 1)) / CARD_ROWS).max(CARD_MIN_HEIGHT);
    ...
    fn draw_header(hdc: HDC, bounds: RectI) {
        unsafe {
            ...
            let subtitle = wide("Step 1: one vertically centered sine wave, tapered to zero at the edges, traveling right-to-left.");

            let _ = DrawTextW(hdc, title.as_ptr(), -1, &mut title_rect, DT_SINGLELINE | DT_VCENTER);
            let _ = SetTextColor(hdc, rgb(92, 86, 79) as COLORREF);
            let _ = DrawTextW(hdc, subtitle.as_ptr(), -1, &mut subtitle_rect, DT_SINGLELINE | DT_VCENTER);
        }
    }

    fn draw_card(graphics: *mut GpGraphics, hdc: HDC, card: RectI, indicator: IndicatorSpec, t: f32) {
        unsafe {
        ...
        unsafe {
            ...
            let _ = DrawTextW(hdc, number.as_ptr(), -1, &mut number_rect, DT_CENTER | DT_SINGLELINE | DT_VCENTER);
            let _ = SetTextColor(hdc, rgb(111, 101, 91) as COLORREF);
            let _ = DrawTextW(hdc, name.as_ptr(), -1, &mut name_rect, DT_CENTER | DT_SINGLELINE | DT_VCENTER);
        }
    }
    ...
    fn draw_halo_soft(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        stroke_circle(graphics, center_circle(rect, ring), argb(alpha, 236, 84, 67), 2.0);
    ...
    fn draw_halo_tight(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        stroke_circle(graphics, center_circle(rect, ring), argb(alpha, 236, 84, 67), 2.6);
    ...
    fn draw_halo_beacon(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for idx in 0..3 {
            ...
            stroke_circle(graphics, center_circle(rect, ring), argb(alpha, 236, 84, 67), 1.8);
        }
    ...
    fn draw_halo_orbit(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        fill_circle(
            graphics,
            RectI { left: x.round() as i32 - 3, top: y.round() as i32 - 3, right: x.round() as i32 + 3, bottom: y.round() as i32 + 3 },
            argb(245, 249, 126, 84),
        );
    }
    ...
    fn draw_halo_bloom(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        fill_circle(graphics, center_circle(rect, glow), argb((20.0 + phase * 52.0).round() as u8, 246, 131, 88));
        fill_circle(graphics, center_circle(rect, 18 + (phase * 8.0).round() as i32), argb((28.0 + phase * 70.0).round() as u8, 240, 100, 74));
    ...
    fn draw_sine_classic(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        draw_sine_curve(graphics, rect, t, 2.2, 10.0, 0.42, 1.2, 0.0, argb(235, 219, 80, 63), 2.0);
    }

    fn draw_sine_fast(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        draw_sine_curve(graphics, rect, t, 3.0, 11.0, 0.54, 1.5, 0.0, argb(240, 229, 86, 64), 2.0);
    }

    fn draw_sine_double(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        draw_sine_curve(graphics, rect, t, 2.1, 8.0, 0.38, 1.8, 0.0, argb(210, 224, 82, 63), 1.8);
        draw_sine_curve(graphics, rect, t, 2.1, -8.0, 0.38, 1.8, 0.0, argb(135, 224, 82, 63), 1.8);
    }
    ...
    fn draw_sine_dots(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        for step in 0..18 {
            ...
            let y = rect.center_y() as f32 + 10.0 * envelope * ((t * 2.7 + step as f32 * 0.55).sin());
            let phase = pulse_phase(t, 0.9, step as f32 * 0.25);
            let size = 3 + (phase * 3.0).round() as i32;
            fill_circle(
                graphics,
                RectI { left: x - size, top: y.round() as i32 - size, right: x + size, bottom: y.round() as i32 + size },
    ...
    fn draw_sine_ribbon(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        draw_sine_curve(graphics, rect, t, 2.0, 9.0, 0.36, 1.35, 0.0, argb(120, 233, 111, 83), 4.0);
        draw_sine_curve(graphics, rect, t, 2.0, 9.0, 0.36, 1.35, 0.0, argb(245, 224, 82, 63), 2.0);
    }

    fn draw_sine_sweep(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        draw_sine_curve(graphics, rect, t, 2.25, 10.0, 0.44, 1.6, 0.0, argb(220, 220, 80, 63), 2.0);
        let sweep = pulse_phase(t, 0.68, -0.8);
        let x = rect.left + 8 + (sweep * (rect.width() - 16) as f32).round() as i32;
        fill_circle(
            graphics,
            RectI { left: x - 5, top: rect.center_y() - 5, right: x + 5, bottom: rect.center_y() + 5 },
            argb(240, 245, 137, 88),
        );
    }
    ...
    fn draw_sine_curve(
        ...
        for step in 0..28 {
            ...
            let envelope = (progress * std::f32::consts::PI).sin().max(0.0).powf(taper_power);
            let y = rect.center_y() as f32
                + amplitude * envelope * ((t * speed + step as f32 * wavelength + phase_offset).sin());
    ...
    fn draw_twin_dots(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        let spacing = 18;
        let left = RectI { left: rect.center_x() - spacing - 5, top: rect.center_y() - 5, right: rect.center_x() - spacing + 5, bottom: rect.center_y() + 5 };
        let right = RectI { left: rect.center_x() + spacing - 5, top: rect.center_y() - 5, right: rect.center_x() + spacing + 5, bottom: rect.center_y() + 5 };
        fill_circle(graphics, left, argb((60.0 + pulse_phase(t, 0.7, 0.0) * 190.0) as u8, 231, 76, 63));
        fill_circle(graphics, right, argb((60.0 + pulse_phase(t, 0.7, 2.2) * 190.0) as u8, 231, 76, 63));
    }
    ...
    fn draw_three_dots(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        for idx in 0..3 {
            ...
            fill_circle(
                graphics,
                RectI { left: x - size / 2, top: rect.center_y() - size / 2, right: x + size / 2, bottom: rect.center_y() + size / 2 },
    ...
    fn draw_wave_bars(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        for idx in 0..5 {
            ...
            fill_rect(graphics, RectI { left: x, top: rect.center_y() - h / 2, right: x + 7, bottom: rect.center_y() + h / 2 }, argb(230, 222, 79, 60));
        }
    }
    ...
    fn draw_equalizer(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        for idx in 0..4 {
            ...
            fill_rect(graphics, RectI { left: x, top: rect.center_y() - h / 2, right: x + 10, bottom: rect.center_y() + h / 2 }, argb(220, 198, 88, 64));
        }
    }
    ...
    fn draw_spinner(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for idx in 0..8 {
            ...
            let alpha = if idx == active { 255 } else { 70 + (((idx + 8 - active) % 8) * 20) as u8 };
            fill_circle(
                graphics,
                RectI { left: x.round() as i32 - 3, top: y.round() as i32 - 3, right: x.round() as i32 + 3, bottom: y.round() as i32 + 3 },
    ...
    fn draw_orbit(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        stroke_circle(graphics, center_circle(rect, 28), argb(80, 221, 160, 150), 1.5);
        let angle = t * std::f32::consts::TAU * 0.55;
        let x = rect.center_x() as f32 + 14.0 * angle.cos();
        let y = rect.center_y() as f32 + 14.0 * angle.sin();
        fill_circle(graphics, center_circle(rect, 6), argb(120, 234, 89, 69));
        fill_circle(graphics, RectI { left: x as i32 - 5, top: y as i32 - 5, right: x as i32 + 5, bottom: y as i32 + 5 }, argb(255, 234, 89, 69));
    }
    ...
    fn draw_heartbeat(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for pair in points.windows(2) {
            draw_line(graphics, pair[0].0, pair[0].1, pair[1].0, pair[1].1, argb(245, 226, 78, 60), 2.0);
        }
    }
    ...
    fn draw_scanner(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        fill_rect(graphics, RectI { left: x, top: lane.top, right: x + 10, bottom: lane.bottom }, argb(190, 233, 87, 63));
    }
    ...
    fn draw_ripple(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        stroke_circle(graphics, center_circle(rect, ring), argb(alpha, 225, 81, 63), 2.0);
    ...
    fn draw_sine_wave(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for step in 0..26 {
            ...
            let point = (x, y.round() as i32);
            if let Some(prev) = last {
                draw_line(graphics, prev.0, prev.1, point.0, point.1, argb(235, 219, 80, 63), 2.0);
            }
            last = Some(point);
        }
    }
    ...
    fn draw_twin_rings(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        let left = RectI { left: rect.center_x() - 24, top: rect.center_y() - 12, right: rect.center_x(), bottom: rect.center_y() + 12 };
        let right = RectI { left: rect.center_x(), top: rect.center_y() - 12, right: rect.center_x() + 24, bottom: rect.center_y() + 12 };
        stroke_circle(graphics, left, argb((80.0 + pulse_phase(t, 0.55, 0.0) * 170.0) as u8, 228, 82, 63), 2.0);
        stroke_circle(graphics, right, argb((80.0 + pulse_phase(t, 0.55, 1.6) * 170.0) as u8, 228, 82, 63), 2.0);
    }
    ...
    fn draw_bounce(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        fill_rect(graphics, RectI { left: rect.center_x() - 12, top: rect.bottom - 8, right: rect.center_x() + 12, bottom: rect.bottom - 4 }, argb(28, 40, 40, 40));
        fill_circle(graphics, RectI { left: rect.center_x() - 5, top: y - 5, right: rect.center_x() + 5, bottom: y + 5 }, argb(240, 233, 81, 61));
    }
    ...
    fn draw_comet(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for idx in 0..5 {
            ...
            let px = x - idx * 8;
            fill_circle(graphics, RectI { left: px - 4, top: rect.center_y() - 4, right: px + 4, bottom: rect.center_y() + 4 }, argb(alpha, 236, 86, 63));
        }
    }
    ...
    fn draw_chevrons(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        for idx in 0..3 {
            ...
            draw_line(graphics, x - 4, rect.center_y() - 8, x + 4, rect.center_y(), argb(alpha, 224, 82, 62), 2.0);
            draw_line(graphics, x - 4, rect.center_y() + 8, x + 4, rect.center_y(), argb(alpha, 224, 82, 62), 2.0);
        }
    }
    ...
    fn draw_capsules(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        for idx in 0..4 {
            ...
            fill_rect(graphics, RectI { left: x, top: rect.center_y() - 5, right: x + w, bottom: rect.center_y() + 5 }, argb((90.0 + phase * 150.0) as u8, 231, 84, 62));
        }
    }
    ...
    fn draw_radar(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        stroke_circle(graphics, center_circle(rect, 18), argb(45, 203, 197, 190), 1.0);
        let angle = t * 140.0;
        let rad = angle.to_radians();
        let x = rect.center_x() + (14.0 * rad.cos()).round() as i32;
        let y = rect.center_y() + (14.0 * rad.sin()).round() as i32;
        draw_line(graphics, rect.center_x(), rect.center_y(), x, y, argb(170, 229, 86, 64), 2.0);
        fill_circle(graphics, RectI { left: x - 3, top: y - 3, right: x + 3, bottom: y + 3 }, argb(255, 229, 86, 64));
    }
    ...
    fn draw_beacon(graphics: *mut GpGraphics, rect: RectI, t: f32) {
        ...
        stroke_circle(graphics, center_circle(rect, 20), argb(alpha / 2, 235, 86, 63), 2.0);
        stroke_circle(graphics, center_circle(rect, 30), argb(alpha / 3, 235, 86, 63), 2.0);
    ...
    fn fill_rect(graphics: *mut GpGraphics, rect: RectI, color: u32) {
        unsafe {
            ...
            let _ = GdipFillRectangleI(graphics, brush as *mut GpBrush, rect.left, rect.top, rect.width(), rect.height());
    ...
    fn fill_circle(graphics: *mut GpGraphics, rect: RectI, color: u32) {
        unsafe {
            let mut brush = ptr::null_mut();
            let _ = GdipCreateSolidFill(color, &mut brush);
            let _ = GdipFillEllipseI(graphics, brush as *mut GpBrush, rect.left, rect.top, rect.width(), rect.height());
            let _ = GdipDeleteBrush(brush as *mut GpBrush);
        }
    }
    ...
    fn stroke_circle(graphics: *mut GpGraphics, rect: RectI, color: u32, width: f32) {
        unsafe {
            ...
            let _ = GdipDrawEllipseI(graphics, pen, rect.left, rect.top, rect.width(), rect.height());
            let _ = GdipDeletePen(pen);
        }
    }

    fn stroke_arc(graphics: *mut GpGraphics, rect: RectI, start: f32, sweep: f32, color: u32, width: f32) {
        unsafe {
            let mut pen = ptr::null_mut();
            let _ = GdipCreatePen1(color, width, UnitPixel, &mut pen);
            let _ = GdipSetPenLineJoin(pen, LineJoinRound);
            let _ = GdipDrawArcI(graphics, pen, rect.left, rect.top, rect.width(), rect.height(), start, sweep);
            let _ = GdipDeletePen(pen);
        }
    }

    fn draw_line(graphics: *mut GpGraphics, x1: i32, y1: i32, x2: i32, y2: i32, color: u32, width: f32) {
        unsafe {
            let mut pen = ptr::null_mut();
            let _ = GdipCreatePen1(color, width, UnitPixel, &mut pen);
            let _ = GdipSetPenLineJoin(pen, LineJoinRound);
    ...
    unsafe fn build_round_rect_path(path: *mut GpPath, rect: RectI, radius: i32) -> bool {
        ...
        GdipAddPathLine(path, left + radius, top, right - radius, top) == 0
            ...
            && GdipAddPathArc(path, right - diameter, bottom - diameter, diameter, diameter, 0.0, 90.0) == 0
            && GdipAddPathLine(path, right - radius, bottom, left + radius, bottom) == 0
            && GdipAddPathArc(path, left, bottom - diameter, diameter, diameter, 90.0, 90.0) == 0
